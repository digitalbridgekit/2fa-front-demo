    ...
    window.web3.eth.net.getId()
      .then(networkId => {
        if (networkId.toString() !== '3') {
          this.setState({ error: 'Wrong network. Please connect to Ropsten network.' });
        }
        else {
          this.setState({
            status: "BUSY",
          ...
          this.clientContract.methods.requestGAPINCheck('alice', this.state.value)
            .send({ from: this.state.sourceAddress })
